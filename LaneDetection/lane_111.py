...
def draw_lanes_and_arrow(frame, lines):
    try:
        ...
        if len(lines) == 1:
            # Single lane detected, draw arrow along this lane
            ...
            # Compute dx and dy to always point up
            dy = -arrow_length  # Always move upwards
            dx = int(dy / slope) if abs(slope) > 1e-3 else 0  # Adjust x accordingly

            x_arrow, y_arrow = x1, y1
            x_start, y_start = x1 + dx, y1 + dy

            cv2.arrowedLine(frame, (x_start, y_start), (x_arrow, y_arrow), (0, 0, 255), 3, tipLength=0.3)
            ...
            if slope1*slope2 > 0 :
                ...
                avg_slope = math.tan(90 - ((math.atan(slope1) + math.atan(slope2) )/ 2))
            ...
            # Compute dx and dy to always point up
            dy = -arrow_length  # Always move upwards
            dx = int(dy / avg_slope) if abs(avg_slope) > 1e-3 else 0  # Adjust x accordingly
...
if __name__ == "__main__":
    input_video = r"C:\Users\yagni\PythonProjects\Pathpal\Path_navigation\diversion.mp4"  # Update with the correct path
    output_video = r"C:\Users\yagni\PythonProjects\Pathpal\Path_navigation\output4.mp4"
